...
import com.example.huangyouqiang.familyvideochat.AVChatStartContextCallBack;
...
class AVContextControl {
	...
	private static int DEMO_SDK_APP_ID = 1400007396;
	private static String DEMO_ACCOUNT_TYPE = "3672";
	...
	private AVChatStartContextCallBack avChatStartContextCallBack;//自己的回调
	...
	private AVContext.StartContextCompleteCallback mStartContextCompleteCallback = new AVContext.StartContextCompleteCallback() {
		public void OnComplete(int result) {
			...
			if(result == AVError.AV_OK){
				avChatStartContextCallBack.onStartContextSuccessful();
			}
			...
			if (result != AVError.AV_OK) {
				avChatStartContextCallBack.onStartContextFailed();
				mAVContext = null;
	 ...
	 *            用户身份的校验信息
	 */
	int startContext(String identifier, String usersig, AVChatStartContextCallBack callBack) {
		int result = AVError.AV_OK;

		avChatStartContextCallBack = callBack;
		if (!hasAVContext()) {			
